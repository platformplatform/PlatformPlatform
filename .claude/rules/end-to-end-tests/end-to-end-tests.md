...
8. Dropdown Menu Animation Handling:
   - When clicking items in animated dropdown menus (DropdownMenu, user profile menu, etc.), use `.evaluate((el: HTMLElement) => el.click())` to bypass Playwright's stability checks
   - This is required because the DropdownMenu component has a 100ms animation that causes Firefox to report "element is not stable"
...
```typescript
// ✅ DO: Use JavaScript evaluate for menu item clicks
