...
4. Search the codebase for ALL similar and related patterns. For example, if the task introduces a new convention for command handlers, find every command handler to verify the pattern was applied consistently. Spin up parallel sub-agents to search broadly. Build your checklist from the codebase, not the task description
...
12. **Stage approved files**: notify the Guardian to stage each approved file. Verify with `git status` that all reviewed files are staged before proceeding
13. **Final handoff**: notify the Guardian that all files are approved and ready for final validation and commit

## File-by-File Staging

When you approve a file, notify the Guardian to stage it: "Stage [file path]". This signals approval:
