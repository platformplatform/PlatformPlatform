...
4. Search the codebase for ALL similar and related patterns. For example, if the task introduces optimistic updates on some mutations, find every similar mutation to verify the pattern was applied everywhere. Spin up parallel sub-agents to search broadly. Build your checklist from the codebase, not the task description
...
14. **Stage approved files**: notify the Guardian to stage each approved file. Verify with `git status` that all reviewed files are staged before proceeding
15. **Final handoff**: notify the Guardian that all files are approved and ready for final validation and commit
