...
11. **Guardian commits**: Guardian runs final validation, makes commits in dependency order (backend before frontend when frontend depends on backend, then E2E), moves [tasks] to [Completed]. Guardian proactively restarts Aspire when backend changes are approved
...
- **SendMessage**: Normal communication. Target agent processes it when their current turn ends
- **Interrupt (SendInterruptSignal + SendMessage)**: Active agents see it immediately via the hook. Idle agents wake from the SendMessage. Agents can send interrupts directly to each other
