...
15. **Stage approved files**: do NOT stage until ALL tests pass. Notify the Guardian to stage each approved test file. Verify with `git status` that all files are staged
16. **Final handoff**: notify the Guardian that E2E files are approved and ready to commit
