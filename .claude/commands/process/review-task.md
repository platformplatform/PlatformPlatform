...
```json
{
  "todos": [
    ...
    {"content": "Run validation tools in parallel (format, test, lint)", "status": "pending", "activeForm": "Running validation tools"},
...
**Lint findings block merging**: If lint returns "Issues found", the CI pipeline will fail and the code cannot be merged. The severity level (note/warning/error) is irrelevant - all findings must be fixed before approval.

**For backend-reviewer** (validates all self-contained systems to catch cross-self-contained-system breakage):

1. Run **build**, **format**, **test**, **lint** following the global tool execution instructions.
...
1. Run **build**, **format**, **lint** for frontend following the global tool execution instructions.
...
- ✓ All validation tools passed (build, format, test, lint).
