...
2. Understand the intention behind the changes instead of the technical details and use that to write the commit message:
...
3.Never add a co-author and never add a description for the commit (one line only).


4. Build and test the codebase (unless you just did it):
...
5. Format the codebase (unless you just did it):
   - If changes have been made to backend `*.cs`, run:
      ```bash
