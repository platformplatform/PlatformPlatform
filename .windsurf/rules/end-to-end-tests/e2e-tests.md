...
3. Organize tests in a consistent file structure:
  ...
  - `@slow` tests:
    ...
    - Must use `test.describe.configure({ timeout: 360000 }); // 6 minutes timeout` to override the default timeout.
...
4. Structure each test with clear *steps*, assertions, and proper monitoring:
   ...
   - Use "Act & Assert:" comment format for all test steps: `// Act & Assert: Do something & verify expected result`
   - The & symbol in comments makes it impossible to miss that every step must have both an action and assertion.
...
test.describe("@smoke", () => {
    ...
    // Act & Assert: Navigate from homepage to signup page & verify navigation completes
    await page.goto("/");
    ...
    // Act & Assert: Enter credentials & verify validation message appears
    ...
    // Act & Assert: Enter verification code & verify successful login
...
test.describe("@security", () => { // ❌ DON'T: Don't invent new tags
  test("should handle login", async ({ page }) => {
    ...
    // Navigate to login page  // ❌ DON'T: Don't add step comments without "Act & Assert:" prefix and expected result   
