...
1. **SPA Served by .NET Backend**:
   - SPA are served via `SinglePageAppFallbackExtensions.cs` from the backend.
...
2. **Module Federation for Micro-Frontends**:
   ...
   - Use `window.location.href` for navigation between self-contained systems (not TanStack Router).
...
1. Follow these code style and pattern conventions:
   ...
   - Never use acronyms in code (e.g., use `errorMessage` not `errMsg`, `button` not `btn`, `authentication` not `auth`, `navigation` not `nav`).
   - Prioritize code readability and maintainability.
   - Never introduce new npm dependencies.
   - Always use React Aria Components instead of native HTML elements like: `<a>`, `<button>`, `<fieldset>`, `<form>`, `<h1>` to `<h6>`, `<img>`, `<input>`, `<label>`, `<ol>`, `<p>`, `<progress>`, `<select>`, `<table>`, `<textarea>`, `<ul>`. Native `<div>`, `<span>`, `<section>`, and `<article>` are acceptable for layout and semantic structure.
...
2. Use the following React patterns and libraries:
   ...
   - Colocate state with components - do not lift state unnecessarily.
...
3. Error handling:
   - **Errors are handled globally** - `shared-webapp/infrastructure/http/errorHandler.ts` automatically shows toast notifications with the server's error message. Do not manually show toasts for errors.
...
function BadAvatarMenu({ userInfo }) { // Bad: Mixing menu with logout functionality
  return (
    <div className="menu"> // ❌ DON'T: use css styles instead of Tailwind
      ...
      <button onClick={() => {
        // ❌ DON'T: Implement logout logic directly in the component, and do not call fetch directly
