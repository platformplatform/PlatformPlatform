...
3. Use strong typing:
   - Whenever possible, use types from the API instead of creating new types.
     ```TypeScript
     import { UserRole, type components } from "@/shared/lib/api/client";
     
     const userRole: UserRole = "admin";
     type UserDetails = components["schemas"]["UserDetails"];
     ```

4. Always follow these steps when implementing changes:
...
5. Build and format your changes:
   - After each minor change, run `[CLI_ALIAS] build --frontend` to build your code. See [Tools](/.windsurf/rules/tools.md) for details.
   - This ensures consistent code style across the codebase.
   - When you get linting errors like `These CSS classes should be sorted.` running `[CLI_ALIAS] format --frontend` will often fix it.

6. Verify your changes:
   - When a feature is complete, run `[CLI_ALIAS] check --frontend` to verify your changes. This command runs build, format, and check in that order.
...
export function AvatarMenu({ userDetails, onProfileClick, onLogoutClick }: AvatarMenuProps) {
  return (
    <Menu placement="bottom end">
      <MenuHeader>
        <div className="flex flex-row items-center gap-2">
          <Avatar avatarUrl={userDetails.avatarUrl} initials={userDetails.initials} isRound={true} size="sm" />
          <div className="my-1 flex flex-col">
            <h2>{userDetails.fullName}</h2>
            <p className="text-muted-foreground">{userDetails.title ?? userDetails.email}</p>
          </div>
        </div>
      </MenuHeader>
...
function BadAvatarMenu({ userDetails }) { // Bad: Mixing menu with logout functionality
  return (
    <div className="menu"> // ❌ DON'T: use css styles instead of Tailwind
      <div className="header">
        <div className="flex flex-row items-center gap-2"> // ❌ DON'T: <div> as the only element inside another <div>
          <div className="m-12">
            <img src={userDetails.avatarUrl} alt="User avatar" /> // ❌ DON'T: Use native <img>
            <h2>{userDetails.fullName}</h2>
          </div>
        </div>
      </div>
