    ...
    <Modal isOpen={isOpen} onOpenChange={onOpenChange} isDismissable={!isPending}> // ✅ Prevent dismiss during pending
      <Dialog className="sm:w-dialog-md"> // ✅ Use dialog width classes (not max-w-lg)
        {({ close }) => ( // ✅ Dialog render prop provides close function
          <>
            <XIcon onClick={close} className="absolute top-2 right-2 h-10 w-10 cursor-pointer p-2 hover:bg-muted" /> // ✅ Close button pattern
            ...
            <DialogFooter>
              <Button variant="default" onClick={handleConfirm} disabled={isPending}> // ✅ ShadCN: variant="default", onClick, disabled
                {isPending ? <Trans>Confirming...</Trans> : <Trans>Confirm</Trans>}
              </Button>
            </DialogFooter>
          </>
        )}
  ...
  return (
    <Modal isOpen={isOpen} onOpenChange={onClose}> // ❌ Missing isDismissable={!isPending}
      <Dialog className="sm:max-w-lg bg-white"> // ❌ max-w-lg (use w-dialog-md), hardcoded colors (use bg-background)
        ...
        <Button variant="primary" onPress={handleSelect}>Submit</Button> // ❌ React Aria patterns (use variant="default", onClick, disabled)
      </Dialog>
    </Modal>
  );
}
