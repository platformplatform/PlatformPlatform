...
7. After changing the API, use the **build MCP tool** for backend to generate the OpenAPI JSON contract. Then use the **build MCP tool** for frontend to trigger `openapi-typescript` to generate the API contract used by the frontend. See [Tools](/.windsurf/rules/tools.md) for details.
