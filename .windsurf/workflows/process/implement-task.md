...
**Note:** The [taskId] comes from current-task.json, not from command arguments. The CLI passes only the [taskTitle] as the slash command argument.
...
Since [tasks] are complete vertical slices, you MUST build and test incrementally as you work through each subtask. This prevents accumulating errors and makes debugging easier.
...
3. **CRITICAL for backend engineers**: Check `git status` for any `*.Api.json` files and include them in your file list
4. List YOUR files in "Files Changed" section (one per line with status)
...
**STEP 12**: Check feature progress (optional)
...
**STEP 13**: Signal completion and exit
