...
Your job is to work with the user through an interactive wizard to create a high-level PRD using language that is easy to understand for non-technical people. The PRD defines a [feature] with all [slices] to be created in `[PRODUCT_MANAGEMENT_TOOL]`.
...
- multiSelect: false
- options:
  - label: "Yes", description: "Include E2E tests as a separate slice"
...
```
AskUserQuestion with:
- question: "Should slices be optimized for parallel work of backend and frontend?"
- header: "Parallel"
- multiSelect: false
- options:
...
2. **Slices section** structured based on wizard answers:
   ...
   **Note:** These are examples only. Adapt the slice structure to match the actual feature requirements, scope, and user answers.

3. **Slice guidelines:**
   - Each slice should be a logical grouping (e.g., "all backend", "all frontend", "all e2e tests")
   - Keep slices small enough to review effectively
   - Write a clear paragraph describing what each slice delivers
   - Each task = one commit = one vertical slice
   - All tasks in a slice work together to deliver a coherent feature
   - **List slices in implementation order** (the order they should be implemented)
   - E2E tests should typically be the final slice
   - **Important:** When using MCP-based `[PRODUCT_MANAGEMENT_TOOL]`, create slices in the same order they appear in the PRD—this defines the implementation sequence

Show the complete PRD to the user - display the full content including all slices with their descriptions.
...
- [slice] for each slice with title=[slice title], description=[slice description], link to [feature], assign to "me"
- Initialize all items in [Planned] status

**Inform user:** The [feature] has been created from the PRD. Use `/process:plan-slices` to break down [slices] into detailed [tasks].
...
- Create [feature] or [slices] in `[PRODUCT_MANAGEMENT_TOOL]` before getting PRD approval in Step 5
- Rename the file—must be `prd.md`
- Save questions in the PRD file
- Create [slices] that split tests, implementation, and migrations across separate [slices]
