...
# Project Overview
PlatformPlatform is an enterprise-grade, multi-tenant foundation for SaaS startups. It employs a self-contained systems (SCS) architecture with micro-frontends, ensuring scalability and maintainability.
...
│  ├─ shared-webapp      # Reusable components that affect all SCSs. Uses React Aria Components
...
- Adhere to the vertical slice architecture pattern. New features are created in the /Features subfolder in the SCS .Core project.
- The @application/shared-kernel/SharedKernel is shared between all SCSs, it contains many reusable components like Aggregates, RepositoryBase, etc.
- Write unit tests with xUnit and Fluent Assertions, following this naming convention: [Method_WhenX_ShouldY].

# Frontend Guidelines
- Develop using React with TypeScript.
- Employ React Aria Components for accessible UI elements. Use the styled components in this folder @application/shared-webapp/ui/components
- Use RsPack and RsBuild for efficient bundling and building processes.
- Implement micro-frontends within the SCS framework.
- Ensure all UI components are fully localized and accessible, using <Trans>text</Trans> in React or t`text` in text.
- When making API calls use the @application/shared-webapp/infrastructure/api/PlatformApiClient.ts like this:
  await api.get("/api/account-management/users/{id}", { params: { path: { id: userId } } });
- When adding new dependencies use specefic versions without using ^ or ~.
