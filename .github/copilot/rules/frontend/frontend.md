...
3. **API Integration**:
   - **Reads**: Use TanStack DB live queries via Electric SQL sync for all entity reads (users, tenants, subscriptions, sessions). See [tanstack-db-electric-sync.md](/.github/copilot/rules/frontend/tanstack-db-electric-sync.md) for patterns
