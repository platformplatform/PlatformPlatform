...
public class CreateTestDataCommand : Command
{
    ...
    private async Task<bool> IsWebSiteRunning()
    {
        try
        {
            ...
            var response = await HttpClient.GetAsync("/api/account-management/account-registrations/is-subdomain-free?Subdomain=foo");
    ...
    private async Task CreateTestData()
    {
        ...
        try
        {
            ...
            {
                var (tenantId, domainName) = await CreateTenant();
                _createUserTasks = new List<Task>();

                for (var j = 1; j < i * 250; j++)
                {
                    var index = j;
    ...
    private async Task<(string tenantId, string domainName)> CreateTenant()
    {
        try
        {
            ...
            var startAccountRegistrationResponse = await HttpClient.PostAsJsonAsync(
                "/api/account-management/account-registrations/start",
                new { Subdomain = tenantId, Email = email }
            );
            await ThrowIfRequestFailed(startAccountRegistrationResponse);

            var startAccountRegistrationResult = (await startAccountRegistrationResponse.Content.ReadFromJsonAsync<AccountRegistrationStart>())!;
            var completeAccountRegistrationResponse = await HttpClient.PostAsJsonAsync(
                $"/api/account-management/account-registrations/{startAccountRegistrationResult.AccountRegistrationId}/complete",
    ...
    public async Task CreateUser(string tenantId, string domainName, bool skipName, bool skipTitle, bool makeOwner, bool makeAdmin)
    {
        try
        {
            ...
            if (_uniqueEmails!.TryAdd(email, 0) == false) goto retry; // A goto statement is more readable than a while loop :)
    ...
    private async Task ThrowIfRequestFailed(HttpResponseMessage responseMessage)
    {
        if (!responseMessage.IsSuccessStatusCode) throw new Exception(Environment.NewLine + await responseMessage.Content.ReadAsStringAsync());
    }
}

public record AccountRegistrationStart(string AccountRegistrationId);
