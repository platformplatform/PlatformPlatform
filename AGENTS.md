...
Always use MCP tools (`build`, `test`, `format`, `inspect`, `run`, `end_to_end`) instead of running dotnet/npm/npx commands directly. Run `build` first, then remaining tools with `noBuild=true`.
