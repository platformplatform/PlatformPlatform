...
export default function AccountMenu({ isCollapsed: isCollapsedProp }: Readonly<AccountMenuProps>) {
  ...
  const triggerClassName = `relative flex h-11 cursor-pointer items-center gap-0 overflow-visible rounded-md border-0 bg-transparent py-2 font-normal text-sm outline-ring hover:bg-hover-background focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 ${
      ...
      <DropdownMenu onOpenChange={setIsMenuOpen}>
        ...
        >
          ...
          {sortedTenants.length > 1 && (
            <DropdownMenuSub>
              <DropdownMenuSubTrigger className="py-3.5">
          ...
          <DropdownMenuSub>
            <DropdownMenuSubTrigger className="py-3.5">
              ...
              <span className="text-muted-foreground text-xs">{currentLocaleLabel}</span>
            </DropdownMenuSubTrigger>
            <DropdownMenuSubContent>
              {locales.map((locale) => (
