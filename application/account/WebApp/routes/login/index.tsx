...
export function LoginForm() {
  ...
  const handleGoogleLogin = () => {
    ...
    const loginUrl = returnPath
      ? `/api/account/authentication/${ExternalProviderType.Google}/login/start?returnPath=${encodeURIComponent(returnPath)}`
      : `/api/account/authentication/${ExternalProviderType.Google}/login/start`;
    window.location.href = loginUrl;
  };

  if (startLoginMutation.isSuccess) {
    const { emailLoginId, emailConfirmationId, validForSeconds } = startLoginMutation.data;
