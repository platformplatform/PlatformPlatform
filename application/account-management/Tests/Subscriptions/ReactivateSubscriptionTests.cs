...
public sealed class ReactivateSubscriptionTests : EndpointBaseTest<AccountManagementDbContext>
{
    [Fact]
    public async Task ReactivateSubscription_WhenCancelledSamePlan_ShouldSucceed()
    {
        ...
        // Act
        var response = await AuthenticatedOwnerHttpClient.PostAsJsonAsync("/api/account-management/subscriptions/reactivate", command);
    ...
    public async Task ReactivateSubscription_WhenNonOwner_ShouldReturnForbidden()
    {
        ...
        // Act
        var response = await AuthenticatedMemberHttpClient.PostAsJsonAsync("/api/account-management/subscriptions/reactivate", command);
