...
public sealed class UpgradeSubscriptionTests : EndpointBaseTest<AccountManagementDbContext>
{
    [Fact]
    public async Task UpgradeSubscription_WhenStandardToPremium_ShouldSucceed()
    {
        ...
        // Act
        var response = await AuthenticatedOwnerHttpClient.PostAsJsonAsync("/api/account-management/subscriptions/upgrade", command);
    ...
    public async Task UpgradeSubscription_WhenNonOwner_ShouldReturnForbidden()
    {
        ...
        // Act
        var response = await AuthenticatedMemberHttpClient.PostAsJsonAsync("/api/account-management/subscriptions/upgrade", command);
