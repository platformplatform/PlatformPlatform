...
public sealed class CheckoutSuccessTests : EndpointBaseTest<AccountManagementDbContext>
{
    [Fact]
    public async Task CheckoutSuccess_WhenValidSession_ShouldSyncSubscription()
    {
        ...
        // Act
        var response = await AuthenticatedOwnerHttpClient.PostAsJsonAsync("/api/account-management/subscriptions/checkout-success", command);
    ...
    public async Task CheckoutSuccess_WhenNonOwner_ShouldReturnForbidden()
    {
        ...
        // Act
        var response = await AuthenticatedMemberHttpClient.PostAsJsonAsync("/api/account-management/subscriptions/checkout-success", command);
