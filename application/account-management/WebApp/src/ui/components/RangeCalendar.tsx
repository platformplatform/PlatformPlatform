...
export function RangeCalendar<T extends DateValue>(
  { errorMessage, ...props }: Readonly<RangeCalendarProps<T>>
) {
  return (
    <AriaRangeCalendar {...props}>
      ...
      <CalendarGrid className="[&_td]:px-0">
        ...
        <CalendarGridBody>
          {date => (
            <CalendarCell date={date} className="group w-9 h-9 text-sm outline outline-0 cursor-default outside-month:text-gray-300 selected:bg-blue-100 dark:selected:bg-blue-700/30 forced-colors:selected:bg-[Highlight] invalid:selected:bg-red-100 dark:invalid:selected:bg-red-700/30 forced-colors:invalid:selected:bg-[Mark] [td:first-child_&]:rounded-s-full selection-start:rounded-s-full [td:last-child_&]:rounded-e-full selection-end:rounded-e-full">
...
function getSelectionState(isSelected: boolean, isSelectionStart: boolean, isSelectionEnd: boolean): "cap" | "middle" | "none" {
