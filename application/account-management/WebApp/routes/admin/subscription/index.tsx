...
function SubscriptionPage() {
  ...
  const { data: subscription } = api.useQuery("get", "/api/account-management/subscriptions/current", {
    enabled: isOwner
  });
  const { data: stripeHealth } = api.useQuery("get", "/api/account-management/subscriptions/stripe-health", {
    enabled: isOwner
  });

  const checkoutMutation = api.useMutation("post", "/api/account-management/subscriptions/checkout", {
    onSuccess: (data) => {
      if (data.checkoutUrl) {
  ...
  const checkoutSuccessMutation = api.useMutation("post", "/api/account-management/subscriptions/checkout-success", {
    onSuccess: () => {
      setIsProcessing(false);
      queryClient.invalidateQueries({ queryKey: ["get", "/api/account-management/subscriptions/current"] });
      toast.success(t`Your subscription has been activated.`);
    }
  });

  const upgradeMutation = api.useMutation("post", "/api/account-management/subscriptions/upgrade", {
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["get", "/api/account-management/subscriptions/current"] });
  ...
  const downgradeMutation = api.useMutation("post", "/api/account-management/subscriptions/schedule-downgrade", {
    onSuccess: () => {
  ...
  const cancelMutation = api.useMutation("post", "/api/account-management/subscriptions/cancel", {
    onSuccess: () => {
  ...
  const reactivateMutation = api.useMutation("post", "/api/account-management/subscriptions/reactivate", {
    onSuccess: (data) => {
      if (data.checkoutUrl) {
        window.location.href = data.checkoutUrl;
      } else {
