...
test.describe("@smoke", () => {
  ...
  test("should enforce permission-based UI visibility and self-action restrictions", async ({ page }) => {
    ...
    await step("Invite member user and test non-Owner permissions after role switch")(async () => {
      ...
      // Change the invited user to Member role (it defaults to Member but let's be explicit)
      ...
      await expectToastMessage(context, `User role updated successfully for ${member.email}`);
