...
test.describe("@smoke", () => {
   ...
   * - Trial plan display with plan comparison cards
  ...
  test("should handle complete subscription lifecycle with plan changes and billing states", async ({ ownerPage }) => {
    ...
    await step("Navigate to subscription page & verify Trial plan with comparison cards and billing history")(
      async () => {
        ...
        await expect(ownerPage.getByText("Trial", { exact: true }).first()).toBeVisible();

        await expect(ownerPage.getByRole("heading", { name: "Plans" })).toBeVisible();

        const trialCard = ownerPage.locator(".grid > div").filter({ hasText: "Trial" }).first();
        await expect(trialCard.getByText("Free")).toBeVisible();
        await expect(trialCard.getByText("5 users")).toBeVisible();
        await expect(trialCard.getByText("10 GB storage")).toBeVisible();
        await expect(trialCard.getByText("Basic support")).toBeVisible();
        await expect(trialCard.getByRole("button", { name: "Current plan" })).toBeDisabled();
        ...
        await expect(ownerPage.getByText("Billing history")).toBeVisible();
    ...
    await step("Click Subscribe on Standard plan & verify subscription activated with payment history")(async () => {
      ...
      await expect(ownerPage.getByText("No payment history available.")).not.toBeVisible();
    ...
    // === UPGRADE FLOW ===
    await step("Click Upgrade on Premium plan & verify plan upgraded")(async () => {
      const premiumCard = ownerPage.locator(".grid > div").filter({ hasText: "Premium" }).first();
    ...
    // === CANCEL SUBSCRIPTION ===
    await step("Click Cancel subscription & verify confirmation dialog")(async () => {
      await expect(ownerPage.getByRole("heading", { name: "Cancel subscription" }).first()).toBeVisible();
      const cancelButton = ownerPage
        .locator("div")
        .filter({ hasText: "Cancel your subscription" })
        .getByRole("button", { name: "Cancel subscription" });
      await cancelButton.click();

      await expect(ownerPage.getByRole("alertdialog")).toBeVisible();
      await expect(ownerPage.getByText("your account will be suspended")).toBeVisible();
    })();
